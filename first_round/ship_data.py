...
with open('ship_obs.php') as fp:
   for line in fp:
      if '<span style="background-color: #fffff0">SHIP' in line or '<span style="background-color: #f0f8fe">SHIP' in line:
         ...
         if z is None:
            print(line)
         if float(z.group(2)) > -30 and float(z.group(2)) < 40 and float(z.group(3)) < -118 and float(z.group(3)) > -165: 
...
file.write(new_file)
file.close()
shutil.move(filename+'.kml', 'data/'+ filename+'.kml')
os.remove('ship_obs.php')
